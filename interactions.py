...
async def welcome(update: Update, context):
    if update.my_chat_member or not update.chat_member:
        return

    if update.chat_member.new_chat_members:
        for new_member in update.chat_member.new_chat_members:
            if update.chat_member.chat.id not in ALLOWED_IDS:
                continue
